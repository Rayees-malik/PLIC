...
class NewProductsExport extends BaseExport
{
    public function export(Request $request)
    {
        ...
        $spreadsheet = $this->loadFile('templates/new_products.xlsx');
        ...
        $newProductsSheet->setCellValue('A2', "{$sinceDate->toFormattedDateString()} - {$today->toFormattedDateString()}");
        ...
        // All Products
        $productBrands = Product::withAccess()->catalogueActive($includeNonCatalogue)->forExport()
            ->whereDate('listed_on', '>=', $sinceDate)
            ->with(
                [
                    'uom',
                    'countryOrigin',
        ...
        foreach ($productBrands as $products) {
            ...
            foreach ($products as $product) {
                $newProductsData[] = $this->getData($product);
            }
        }

        $newProductsSheet->fromArray($newProductsData, null, 'A4');

        $dateString = $sinceDate->format('m/d/Y');
        $nowDateString = Carbon::now()->format('m/d/Y');

        return $this->downloadFile($spreadsheet, "new_products_{$dateString}-{$nowDateString}.xlsx");
    }
