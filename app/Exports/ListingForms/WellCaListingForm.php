...
class WellCaListingForm extends SimpleListingForm
{
    protected $template = 'templates/listingforms/wellca.xlsx';

    protected $filename = 'well.ca_listingform.xlsx';
    ...
    public function data($stockIds, $includeNonCatalogue)
    {
        ...
        foreach ($products as $product) {
            ...
            if ($product->category->name == 'Food & Beverage') {
                $this->foodData[] = [
                    ...
                    null,
                    implode(', ', $product->certifications->pluck('name')->toArray()),
                    $price / $minimumSellBy,
                    '$' . $srp,
                    'CAD',
                    null,
    ...
    {
        $spreadsheet->getSheet(2)->fromArray($this->foodData, null, 'A5');
        $spreadsheet->getSheet(2)->setSelectedCell('A5');
        $spreadsheet->setActiveSheetIndex(0);
        $spreadsheet->getSheet(0)->setSelectedCell('A5');
    }
}
